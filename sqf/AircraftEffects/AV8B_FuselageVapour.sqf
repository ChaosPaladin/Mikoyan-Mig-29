...
_positions = [[-1.2,1,0],[1.2,1,0],[-2,0,0],[2,0,0]];
_emitters = [];
...
while {true} do
	{
		...
		_direction = vectordir _this;
		if ((_rate*((speed _this)/1000)) > (10*(10*_looptime))) then {if (_opacity < 1) then {_opacity = _opacity + 0.1*(10*_looptime)}}
		else {if (_opacity > 0) then {_opacity = _opacity - 0.4*(10*_looptime)}};
		for "_i" from 0 to ((count _positions)-1) do {(_emitters select _i) setParticleParams ["\Ca\data\ParticleEffects\ROCKETSMOKE\RocketSmoke","","Billboard",1,0.05,(_positions select _i),(velocity _this),1,1.275,1,0.01,[1,3,10],[[1,1,1,0],[1,1,1,0.02*_opacity],[1,1,1,0.005*_opacity],[1,1,1,0]],[0],0.001,5,"","",_this]};
