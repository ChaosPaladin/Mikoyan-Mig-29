...
while {true} do
	{
		...
		_direction = vectordir _this;
		if ((_rate*((speed _this)/1000)) > (7*(10*_looptime))) then	{if (_opacity < 1) then {_opacity = _opacity + 0.05*(10*_looptime)}}
		else {if (_opacity > 0) then {_opacity = _opacity - 0.2*(10*_looptime)}};
		_leftwingemitter setParticleParams ["\Ca\data\ParticleEffects\ROCKETSMOKE\RocketSmoke","","Billboard",1,1.5,[-4.8,-2.7,-0.9],(velocity _this),1,1.275,1,0.1,[0.4,0.5],[[1,1,1,0.01*_opacity],[1,1,1,0.2*_opacity],[1,1,1,0]],[0],0.01,0.01,"","",_this];
		_rightwingemitter setParticleParams ["\Ca\data\ParticleEffects\ROCKETSMOKE\RocketSmoke","","Billboard",1,1.5,[4.8,-2.7,-0.9],(velocity _this),1,1.275,1,0.1,[0.4,0.5],[[1,1,1,0.01*_opacity],[1,1,1,0.2*_opacity],[1,1,1,0]],[0],0.01,0.01,"","",_this];
