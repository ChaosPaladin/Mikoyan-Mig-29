...
_emitters = [];
{
 _tmpemitter = "#particlesource" createVehicle position _this;
 _emitters = _emitters + [_tmpemitter];
 } forEach _positions;
...
while {(alive _this) and (_this animationPhase "KillFx" == 0)} do
	{
		...
		_direction = vectorDir _this;
		if ((_rate*((speed _this)/1000)) > (8*(10*_looptime))) then
		{
            if (_opacity < 1) then
              {
                _opacity = _opacity + 0.1*(10*_looptime);
               }
		}
		else {
		    if (_opacity > 0) then
		        {
		            _opacity = _opacity - 0.4*(10*_looptime);
		         }
		      };
